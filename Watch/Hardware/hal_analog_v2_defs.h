...
/******************************************************************************/
/*! \file hal_analog_v2_defs.h
 *
 * Pin/Peripheral definitions for Analog Watch Version 2.
 */
/******************************************************************************/

#ifndef HAL_ANALOG_V2_DEFS
#define HAL_ANALOG_V2_DEFS
...
#define OLED_IO_POWER_ENABLE_PIN  ( BIT0 )
#define OLED_IO_POWER_ENABLE_POUT ( P3OUT )
#define OLED_IO_POWER_ENABLE_PDIR ( P3DIR )
#define OLED_IO_POWER_CONFIG()    { P3DIR |= OLED_IO_POWER_ENABLE_PIN; OLED_IO_POWER_DISABLE(); }
#define OLED_IO_POWER_ENABLE()    { P3OUT &= ~OLED_IO_POWER_ENABLE_PIN; }
#define OLED_IO_POWER_DISABLE()   { P3OUT |= OLED_IO_POWER_ENABLE_PIN; }
...
#define BAT_CHARGE_OPEN_DRAIN_BITS  ( BAT_CHARGE_STAT1 | BAT_CHARGE_STAT2 | BAT_CHARGE_PWR_GOOD )
...
#define ACCELEROMETER_INT_DISABLE() { \
  ACCELEROMETER_INT_PIFG &= ~ACCELEROMETER_INT_PIN;    \
  ACCELEROMETER_INT_PIE &= ~ACCELEROMETER_INT_PIN;     \
  ACCELEROMETER_INT_PSEL &= ~ACCELEROMETER_INT_PIN;    \
}
...
#define CONFIG_ACCELEROMETER_PINS_FOR_SLEEP() { \
  ACCELEROMETER_SDA_PSEL &= ~ACCELEROMETER_SDA_PIN;  \
  ACCELEROMETER_SCL_PSEL &= ~ACCELEROMETER_SCL_PIN;  \
  ACCELEROMETER_SDA_PDIR |= ACCELEROMETER_SDA_PIN; \
  ACCELEROMETER_SDA_POUT |= ACCELEROMETER_SDA_PIN; \
  ACCELEROMETER_SDA_PDIR |= ACCELEROMETER_SCL_PIN; \
  ACCELEROMETER_SCL_POUT |= ACCELEROMETER_SCL_PIN; \
}

#define CONFIG_ACCELEROMETER_PINS_FOR_USE() { \
  ACCELEROMETER_SDA_PSEL |= ACCELEROMETER_SDA_PIN;  \
  ACCELEROMETER_SCL_PSEL |= ACCELEROMETER_SCL_PIN;  \
}
...
/* this pin is on the ANALOG schematic but is actually BT_IO2 */
#define BT_CLK_REQ_CONFIG() { }
...
/* I thought this pin should be an input but
 * current was 0.9 ma as input and 0.8 mA when an output ???
 * R17 change did not matter
*/
