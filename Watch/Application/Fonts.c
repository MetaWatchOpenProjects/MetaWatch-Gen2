...
const unsigned char MetaWatchTimeBlockTable[TOTAL_TIME_BLOCK_CHARACTERS][60];
const unsigned char TimeGTable[TIMEG_CHARACTER_NUM][3*28];
...
const unsigned char MetaWatchTimeBlockWidth[TOTAL_TIME_BLOCK_CHARACTERS];
const unsigned char TimeGWidth[TIMEG_CHARACTER_NUM];

static const tFont Font[FONT_NUM] =
{
  {5, 1, 5, 1, 0, MetaWatch5width},
  {7, 1, 7, 1, 0, MetaWatch7width},
  {16, 1, 12, 2, 0, MetaWatch16width},
  {19, 1, 11, 2, 1, MetaWatchTimeWidth},
  {20, 1, 20, 3, 1, MetaWatchTimeBlockWidth},
  {28, 1, 18, 3, 1, TimeGWidth}
};
...
unsigned char MapCharacterToIndex(unsigned char CharIn)
{
  ...
  //Check if CharIn was entered as a MetaWatchTime or numeric character
  if(CharIn < TOTAL_TIME_CHARACTERS)
  {
    //Entered as MetaWatchTime or numeric character. 
    //Need to convert to alphanumeric character
    //CharIn was a MetaWatchTime ' ' character. It requires a different offset.
    //CharIn was another MetaWatchTime character. It requires the
    //ASCII offset.
...
void GetCharacterBitmap(unsigned char Character,unsigned int * pBitmap)
{
  ...
  for (row = 0; row < Font[CurrentType].Height; row++)
  {
    ...
    case MetaWatchTimeBlock:
      pBitmap[row] = MetaWatchTimeBlockTable[index][row];
      break;
...
unsigned char *GetCharacterBitmapPointer(unsigned char Char)
{
  ...
  case MetaWatchTime: pCharBmp = (unsigned char *)MetaWatchTimeTable[i]; break;
  case MetaWatchTimeBlock: pCharBmp = (unsigned char *)MetaWatchTimeBlockTable[i]; break;
...
/*******************************************************************************************
  ...
  To get the starting data offset of character 'A', you can use the following expression:
     ...
     const unsigned short *pData = data_table[offset];
...
const unsigned char MetaWatchTimeBlockTable[TOTAL_TIME_BLOCK_CHARACTERS][60] =
{
  ...
  0xFF, 0x03, 0x00, 0xFC, 0x03, 0x00, 0xFC, 0x03,
  0x00, 0xFC, 0x03, 0x00, 0xFC, 0x03, 0x00, 0xFC,
